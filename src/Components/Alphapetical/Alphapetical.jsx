...
import { SortedGamesContext } from './../../SortedGamesContext/SortedGamesContext';
import { Link } from 'react-router-dom';
import { AllGamesContext } from '../../AllGamesContext/AllGamesContext';


export default function Alphapetical() {
  let {getMoreGames , count} = useContext(AllGamesContext)

let {alphapetical,setAlphapetical , getSortedGames} = useContext(SortedGamesContext)


useEffect(()=> {
  getSortedGames(  'alphabetical',setAlphapetical )
},[])
  ...
  {alphapetical.slice(0,count).map((game , index)=> <div key={index} className="col-md-3 card-list "><Link to={`/GamesDetails/${game.id}`}> <div className="card shadow" >
